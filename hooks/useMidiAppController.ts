...
export const useMidiAppController = () => {
  ...
  const inversionStats = useMemo(() => {
    if (
      !project.midiData ||
      project.selectedTracks.size === 0 ||
      !settings.melodicInversion.enabled
    )
      return null;
    ...
    return calculateInversionStats(track.notes, settings.melodicInversion, ppq, {
      numerator: tsNum,
      denominator: tsDenom,
    });
  }, [
    project.midiData,
    project.selectedTracks,
    settings.melodicInversion,
    settings.newTimeSignature,
  ]);

  const quantizationWarning = useMemo(() => {
    if (!project.midiData || (!settings.primaryRhythm.enabled && !settings.pruneOverlaps))
      return null;
    ...
    project.midiData,
    project.selectedTracks,
    settings.primaryRhythm,
    settings.pruneOverlaps,
  ...
  const handleCombine = async () => {
    ...
    try {
      ...
      if (settings.outputStrategy === 'separate_voices') suffix = '_voices';
        ...
        suffix =
          project.selectedTracks.size === 1
            ? `_track${(Array.from(project.selectedTracks)[0] as number) + 1}`
            : '_combined';

      await combineAndDownload(
        project.midiData,
  ...
  const handlePreviewAbc = () => {
    ...
    try {
      ...
      ui.setSuccessMessage(
        `Prepared ${previews.length} ABC preview${previews.length === 1 ? '' : 's'}.`
      );
      ui.setUiState(AppState.SUCCESS);
    } catch (e) {
      console.error(e);
  ...
  const handleAnalyzeSelection = () => {
    ...
    try {
      const analysis = analyzeTrackSelection(
        project.midiData,
        Array.from(project.selectedTracks),
        getConversionOptions() || undefined
      );
      ui.setAnalysisData(analysis);
      ui.setIsAnalysisVisible(true);
    } catch (e) {
  ...
  return {
    ...
    computed: {
      ...
      quantizationWarning,
      isLoadedState:
        [AppState.LOADED, AppState.COMBINING, AppState.SUCCESS, AppState.DOWNLOAD_ERROR].includes(
          project.loadState
        ) || project.midiData !== null,
    },
    handlers: {
      handleReset,
      handleCombine,
