...
export const useMidiController = () => {
  ...
  const { settings, setters, handleResetSettings, getConversionOptions } =
    useConversionSettings(midiData);
      ...
      if (settings.outputStrategy === 'separate_voices') suffix = '_voices';
        ...
        suffix =
          selectedTracks.size === 1
            ? `_track${(Array.from(selectedTracks)[0] as number) + 1}`
            : '_combined';

      const newFileName = `${baseName}${suffix}.mid`;
      await combineAndDownload(
        midiData,
        Array.from(selectedTracks) as number[],
        newFileName,
  ...
  const handlePreviewTrack = useCallback(
    async (trackId: number) => {
      ...
      if (playingTrackId === trackId) {
        ...
        try {
          const previewMidi = createPreviewMidi(
            midiData,
            trackId,
            settings.eventsToDelete,
            conversionOptions
  ...
  const handleShowPianoRoll = useCallback(
    (trackId: number) => {
      if (!midiData) return;
      clearMessages();
      const conversionOptions = getConversionOptions();
  ...
  const handleEventFilterToggle = useCallback(
