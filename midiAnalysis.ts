...
} from './components/services/midiHarmony';
...
const STANDARD_DURATION_MULTIPLIERS = [
  { name: 'Whole Note', value: 4.0 },
  { name: 'Dotted Half', value: 3.0 },
  { name: 'Half Note', value: 2.0 },
  { name: 'Dotted Quarter', value: 1.5 },
  { name: 'Half Triplet', value: 1.3333 },
  { name: 'Quarter Note', value: 1.0 },
  { name: 'Quarter Quintuplet', value: 0.8 },
  { name: 'Dotted Eighth', value: 0.75 },
  { name: 'Quarter Triplet', value: 0.6666 },
  { name: 'Eighth Note', value: 0.5 },
  { name: 'Eighth Quintuplet', value: 0.4 },
  { name: 'Dotted 16th', value: 0.375 },
  { name: 'Eighth Triplet', value: 0.3333 },
  { name: '16th Note', value: 0.25 },
  { name: '16th Quintuplet', value: 0.2 },
  { name: '16th Triplet', value: 0.1666 },
...
export function generateAnalysisReport(data: TrackAnalysisData): string {
  ...
  topNoteValues.forEach((s, i) => {
    r += `  ${i + 1}. ${s.name} (${Math.round(s.percentage)}%) - ${s.count} notes\n`;
  });
  ...
  const print = (title: string, list: any[]) => {
    ...
    if (!list || list.length === 0) out += `No chords detected.\n`;
      ...
      list.forEach(
        (c) =>
          (out += `${c.formattedTime.padEnd(20)}: ${c.name.padEnd(20)} [${c.constituentNotes.join(', ')}]${c.missingNotes.length ? ` (Missing: ${c.missingNotes.join(', ')})` : ''}\n`)
      );
    return out;
  };
  ...
  if (chordsHybrid?.length)
    r += `\n` + print('CHORD PROGRESSION (Hybrid / Arpeggio)', chordsHybrid);
  if (chordsBucketed?.length)
    r += `\n` + print('CHORD PROGRESSION (Harmonic Rhythm Normalized)', chordsBucketed);

  r += `\n4. VOICE LEADING\n`;
  Object.keys(voiceIntervals)
    .map(Number)
    .sort((a, b) => a - b)
    .forEach(
      (i) =>
        (r += `  ${(i === 0 ? 'Unison' : i > 0 ? `+${i}` : `${i}`).padEnd(8)}: ${voiceIntervals[i]}\n`)
    );
  return r;
}
...
function analyzePreparedNotes(
  ...
  if (notes.length === 0)
    return {
      ...
      durationConsistencyScore: 0,
      averageOffsetTicks: 0,
      totalNotes: 0,
      detectedGridType: 'None',
      pitchClassHistogram: {},
      chordsSustain: [],
      chordsAttack: [],
      notesRaw: [],
  ...
  notes.forEach((note) => {
    ...
    let best = STANDARD_DURATION_MULTIPLIERS[0],
      minD = Math.abs(ratio - best.value);
    for (let i = 1; i < STANDARD_DURATION_MULTIPLIERS.length; i++) {
      const d = Math.abs(ratio - STANDARD_DURATION_MULTIPLIERS[i].value);
      if (d < minD) {
        minD = d;
        best = STANDARD_DURATION_MULTIPLIERS[i];
      }
    }
    if (!durCounts.has(best.name))
      durCounts.set(best.name, {
        name: best.name,
        count: 0,
        percentage: 0,
        standardMultiplier: best.value,
      });
    durCounts.get(best.name)!.count++;
    totalDurConsist += 1 - Math.min(minD / best.value, 1.0);
  });
  const durStats = Array.from(durCounts.values())
    .map((s) => ({ ...s, percentage: (s.count / notes.length) * 100 }))
    .sort((a, b) => b.count - a.count);

  let gridTicks = ts[1] === 8 ? ppq / 2 : ppq / 4,
    gridLabel = ts[1] === 8 ? '1/8 Compound' : '1/16 Standard';
...
export function analyzeTrack(
  ...
  if (distribution.orphans) {
    distribution.orphans.forEach((n) => noteVoiceMap.set(n, -1));
  }
...
export function analyzeTrackSelection(
  midi: Midi,
  trackIds: number[],
  options?: ConversionOptions
): TrackAnalysisData {
