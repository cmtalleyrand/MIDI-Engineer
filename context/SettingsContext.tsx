...
interface SettingsContextType {
  ...
  setters: {
    ...
    setEventsToDelete: (
      val: Set<MidiEventType> | ((prev: Set<MidiEventType>) => Set<MidiEventType>)
    ) => void;
  };
...
export const SettingsProvider = ({ children }: { children?: ReactNode }) => {
  ...
  const [originalTimeSignature, setOriginalTimeSignature] = useState<{
    numerator: number;
    denominator: number;
  } | null>(null);
  ...
  const [melodicInversion, setMelodicInversion] = useState<MelodicInversionOptions>({
    enabled: false,
    startMeasure: 1,
    endMeasure: 4,
  });
  const [exportRange, setExportRange] = useState<ExportRangeOptions>({
    enabled: false,
    startMeasure: 1,
    endMeasure: 8,
  });
  ...
  const [primaryRhythm, setPrimaryRhythm] = useState<RhythmRule>({
    enabled: false,
    family: 'Simple',
    minNoteValue: '1/16',
  });
  const [secondaryRhythm, setSecondaryRhythm] = useState<RhythmRule>({
    enabled: false,
    family: 'Triple',
    minNoteValue: '1/8t',
  });
  ...
  const [keySignatureSpelling, setKeySignatureSpelling] = useState<'auto' | 'sharp' | 'flat'>(
    'auto'
  );
  ...
  const getConversionOptions = useCallback((): ConversionOptions | null => {
    ...
    if (isNaN(parsedTsNum) || isNaN(parsedTsDenom) || parsedTsNum <= 0 || parsedTsDenom <= 0)
      return null;

    const ppq = sourcePPQ || 480;

    const removeThresholdTicks = Math.round(
      ppq * MUSICAL_TIME_OPTIONS[removeShortNotesThresholdIndex].value
    );
  ...
  const setters = {
    ...
    setQuantizationValue: (val: string) => {
      if (val === 'off') setPrimaryRhythm({ ...primaryRhythm, enabled: false });
        ...
        setPrimaryRhythm({ ...primaryRhythm, enabled: true, family: 'Simple', minNoteValue: val });
    },
