...
export function runFixtureSuite() {
  ...
  const tripletOutlier = applyShadowQuantization(
    tripletOutlierInput,
    PPQ,
    primarySimple,
    secondaryTriple
  ...
  const contextualTripletInput = [
    note(70, 0, 160),
    note(72, 160, 160),
    note(74, 320, 160),
    note(76, 500, 170),
  ];
  const contextualTriplet = applyShadowQuantization(
    contextualTripletInput,
    PPQ,
    primarySimple,
    secondaryTriple
  ).map((n) => ({
  ...
  const overlapConflict = applyShadowQuantization(
  ...
  const densityBlip = applyShadowQuantization(
    [note(60, 0, 480), note(64, 240, 40), note(67, 480, 480)],
    PPQ,
    primarySimple,
    secondaryTriple
  ).map((n) => ({
  ...
  const ornaments = {
    grace: detectAndTagOrnaments([note(61, 0, 40), note(60, 40, 240)], PPQ).map((n) => ({
      midi: n.midi,
      isOrnament: !!n.isOrnament,
    })),
    mordent: detectAndTagOrnaments([note(60, 0, 40), note(62, 40, 40), note(60, 80, 300)], PPQ).map(
      (n) => ({ midi: n.midi, isOrnament: !!n.isOrnament })
    ),
    turn: detectAndTagOrnaments(
      [note(62, 0, 40), note(60, 40, 40), note(58, 80, 40), note(60, 120, 300)],
      PPQ
    ).map((n) => ({ midi: n.midi, isOrnament: !!n.isOrnament })),
    trill: detectAndTagOrnaments(
      [note(61, 0, 40), note(60, 40, 40), note(61, 80, 40), note(60, 120, 40), note(60, 160, 300)],
      PPQ
    ).map((n) => ({ midi: n.midi, isOrnament: !!n.isOrnament })),
  };

  const orphanNotes = [note(60, 0, 480), note(64, 240, 480), note(67, 960, 240)];
  const orphanDistribution = distributeToVoices(
    orphanNotes,
    withBaseOptions({ voiceSeparationDisableChords: true, voiceSeparationMaxVoices: 1 }),
    PPQ
  );
  ...
  orphanNotes.forEach((n) =>
    track.addNote({ midi: n.midi, ticks: n.ticks, durationTicks: n.durationTicks, velocity: 0.8 })
  );

  const abc = renderMidiToAbc(
    midi,
    'orphan-suite.abc',
    withBaseOptions({ outputStrategy: 'separate_voices' }),
    120
  );

  const abcCustomPhr = renderMidiToAbc(
    midi,
    'custom-phr.abc',
    withBaseOptions({
      abcKeyExport: {
  ...
  const abcTie = renderMidiToAbc(
    tieMidi,
    'tie.abc',
    withBaseOptions({ outputStrategy: 'combine' }),
    120
  );

  const abcCustomMix = renderMidiToAbc(
    midi,
    'custom-mix.abc',
    withBaseOptions({
      abcKeyExport: {
  ...
  const quantSplit = {
    ...
    explicitQuant: resolveExportOptions(
      withBaseOptions({
