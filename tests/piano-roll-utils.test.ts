...
import {
  canSelectPianoRollNote,
  sortNotesForPianoRollRendering,
} from '../components/services/pianoRollUtils';

type PianoRollNote = PianoRollTrackData['notes'][number];

function buildNote(
  partial: Partial<PianoRollNote> &
    Pick<PianoRollNote, 'name' | 'midi' | 'ticks' | 'durationTicks' | 'velocity'>
): PianoRollNote {
  return {
    name: partial.name,
    midi: partial.midi,
...
export function runPianoRollUtilsTests() {
  assert.equal(
    canSelectPianoRollNote(false, false),
    false,
    'Note selection should be disabled when both debug toggles are off.'
  );
  assert.equal(
    canSelectPianoRollNote(true, false),
    true,
    'Voice debug toggle should enable note selection.'
  );
  assert.equal(
    canSelectPianoRollNote(false, true),
    true,
    'Quantizer debug toggle should enable note selection.'
  );
