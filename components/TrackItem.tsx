    ...
    >
      ...
      <div className="flex-grow grid grid-cols-2 md:grid-cols-4 gap-4 items-center">
        <div className="md:col-span-1">
          <p className="font-bold text-gray-light truncate" title={track.name}>
            {track.name}
          </p>
        ...
        <div className="md:col-span-1 text-right">
          ...
          {track.ornamentCount !== undefined && track.ornamentCount > 0 && (
            <p className="text-xs text-yellow-500 mt-1 font-semibold">
              {track.ornamentCount} Ornaments
            </p>
          )}
        </div>
      </div>
      <div
        className="ml-4 flex-shrink-0 w-6 h-6 flex items-center justify-center border-2 rounded-md transition-all duration-200"
        style={
          {
