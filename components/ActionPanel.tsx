...
import { Spinner, DownloadIcon, AbcIcon, ChartBarIcon, MusicNoteIcon } from './Icons';
    ...
    <div className="w-full bg-gray-dark p-6 rounded-2xl shadow-2xl border border-gray-medium mt-6 animate-slide-up">
        ...
        >
          {isCombining ? (
            <>
              ...
              <span>Processing MIDI...</span>
            </>
          ) : (
            <>
              <DownloadIcon className="w-6 h-6" />
              <span>
                {midiButtonActionText} ({selectedCount} Track{selectedCount !== 1 ? 's' : ''})
              </span>
            </>
          )}
        </button>
          ...
          <div className="grid grid-cols-4 gap-2">
            ...
            >
              {isExportingAbc ? (
                ...
                <>
                  <AbcIcon className="w-4 h-4" />
                  <span className="text-xs">Export ABC</span>
                </>
              )}
            </button>
            <button
            ...
            >
              <AbcIcon className="w-4 h-4" />
              <span className="text-xs">Show ABC</span>
            </button>
            <button
      ...
      {!canProcess && !isCombining && !isExportingAbc && (
        <p className="text-center text-xs text-gray-400 mt-2">
          {' '}
          Select at least 1 track to download or export.{' '}
        </p>
      )}
    </div>
  );
