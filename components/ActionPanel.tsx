...
interface ActionPanelProps {
    ...
    isDrumGenerationEnabled: boolean;
}

export default function ActionPanel({ onCombine, onExportAbc, onAnalyzeSelection, isCombining, isExportingAbc, canProcess, selectedCount, isDrumGenerationEnabled }: ActionPanelProps) {
    ...
    const midiButtonActionText = selectedCount > 0 ? (isDrumGenerationEnabled ? 'Download MIDI + Drums' : 'Download MIDI') : 'Download';
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="flex flex-col gap-2">
                    <div className="grid grid-cols-2 gap-2">
                        <button 
                            onClick={onExportAbc} 
                            disabled={!canProcess || isCombining || isExportingAbc || isDrumGenerationEnabled} 
                        ...
                        >
                            {isExportingAbc ? ( 
                                ...
                                <><AbcIcon className="w-4 h-4" /><span>{isDrumGenerationEnabled ? 'ABC Disabled (Drums ON)' : 'Export ABC'}</span></> 
                            )}
                        </button>
                        <button 
                    ...
                    <div className={`border rounded px-2 py-1 text-center ${isDrumGenerationEnabled ? 'bg-blue-900/30 border-blue-800' : 'bg-yellow-900/30 border-yellow-800'}`}>
                        <p className="text-[10px] text-yellow-500">
                            {isDrumGenerationEnabled ? 'Drum generation is MIDI-only. Turn it off to export ABC.' : <>Warning: ABC spelling defaults to <strong>C Major</strong> if Key is not changed.</>} 
                        </p>
                    </div>
                </div>
            </div>
