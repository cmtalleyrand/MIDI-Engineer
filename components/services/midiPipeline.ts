...
import {
  quantizeNotes,
  performInversion,
  performModalConversion,
  pruneOverlaps,
  performMelodicInversion,
  cropToRange,
} from './midiTransform';
...
export function resolveExportOptions(
  ...
  target: ExportTarget
): { options: ConversionOptions; debug: ExportResolutionDebugInfo } {
  ...
  if (target === 'abc' && !explicitUserQuantization) {
    const primaryRhythmValue =
      options.primaryRhythm.minNoteValue === 'off' ? '1/16' : options.primaryRhythm.minNoteValue;
    return {
      options: {
        ...options,
        quantizationValue: primaryRhythmValue,
        primaryRhythm: {
          ...options.primaryRhythm,
          enabled: true,
          minNoteValue: primaryRhythmValue,
        },
      },
      debug: {
        target,
...
export function copyHeaderEvents(
  ...
  const transformedTS = originalHeader.timeSignatures
    ...
    .map((ts, i) => ({
      ...
      timeSignature:
        i === 0
          ? ([options.timeSignature.numerator, options.timeSignature.denominator] as [
              number,
              number,
            ])
  ...
  if (cropEnabled) {
    ...
    for (const ts of transformedTS) {
      if (ts.ticks > cropStartTick && ts.ticks < cropEndTick) {
        destHeader.timeSignatures.push({
          ticks: ts.ticks - cropStartTick,
  ...
  if (destHeader.timeSignatures.length === 0) {
    destHeader.timeSignatures = [
      {
        ticks: 0,
        timeSignature: [options.timeSignature.numerator, options.timeSignature.denominator],
      },
    ];
  }
...
export function copyAndTransformTrackEvents(
  ...
  const maxTick =
    transformedNotes.length > 0
      ? Math.max(...transformedNotes.map((n) => n.ticks + n.durationTicks))
      : 0;

  // 5. Retrograde (Time Inversion)
  transformedNotes = performInversion(
    transformedNotes,
    options.inversionMode,
    destPPQ,
  ...
  // 6. Melodic Inversion
  transformedNotes = performMelodicInversion(
    transformedNotes,
    options.melodicInversion,
    destPPQ,
    options.timeSignature
  );
  ...
  transformedNotes = dedupeNotesAndReport(
    transformedNotes,
    sourceTrack.name,
    options.timeSignature,
    destPPQ
...
export function createPreviewMidi(
  originalMidi: Midi,
  trackId: number,
  eventsToDelete: Set<MidiEventType>,
  options: ConversionOptions
): Midi {
  if (trackId < 0 || trackId >= originalMidi.tracks.length)
...
export function getTransformedTrackDataForPianoRoll(
  originalMidi: Midi,
  trackId: number,
  options: ConversionOptions
): PianoRollTrackData {
