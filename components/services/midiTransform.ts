...
export function quantizeNotes(notes: any[], options: ConversionOptions, ppq: number): any[] {
  ...
  if (options.primaryRhythm.enabled) {
    notesToProcess = applyShadowQuantization(
      notesToProcess,
      ppq,
      options.primaryRhythm,
      options.secondaryRhythm
    );
    ...
    if (quantizationTicks > 0 || minDurationTicks > 0) {
      notesToProcess.forEach((note) => {
        ...
        if (quantizationTicks > 0) {
          ...
          note.durationTicks =
            Math.round(note.durationTicks / quantizationTicks) * quantizationTicks;
        }
        if (minDurationTicks > 0) {
          note.durationTicks = Math.max(note.durationTicks, minDurationTicks);
        }
        if (note.durationTicks === 0 && quantizationTicks > 0)
          note.durationTicks = quantizationTicks;
      });
    }
  }
...
export function getTransformedNotes(notes: any[], options: ConversionOptions, ppq: number): any[] {
  ...
  const maxTick =
    processed.length > 0 ? Math.max(...processed.map((n) => n.ticks + n.durationTicks)) : 0;
  processed = performInversion(
    processed,
  ...
  processed = performMelodicInversion(
    processed,
