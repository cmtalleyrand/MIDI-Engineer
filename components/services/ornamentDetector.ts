...
const getGapAfter = (a: OrnamentAnnotatedNote, b: OrnamentAnnotatedNote): number =>
  b.ticks - getEnd(a);
 ...
 */
export function getDefaultOrnamentDetectionParams(
  ppq: number,
  familyMNVticks?: number
...
function mkTimingBounds(
  member: OrnamentAnnotatedNote[],
  principal: OrnamentAnnotatedNote
): OrnamentTimingBounds {
...
export function detectOrnamentHypotheses(
  notes: any[],
  params: OrnamentDetectionParams
): OrnamentHypothesis[] {
  ...
  for (let i = 0; i < sorted.length - 1; i++) {
    ...
    if (
      graceIsShort &&
      gap <= params.attachGapTicks &&
      g.durationTicks <= params.ornamentMaxSpanTicks &&
      neighborish
    ) {
      const durScore = cap01(
        1 - g.durationTicks / (Math.min(params.graceMaxDurTicks, maxRelativeDur) + 1)
      );
  ...
  for (let i = 0; i <= sorted.length - 4; i++) {
    ...
    const inRange =
      Math.abs(aDiff) <= params.neighborMaxSemitones &&
      Math.abs(cDiff) <= params.neighborMaxSemitones;
    ...
    if (
      ...
      shortEnough &&
      longPrincipalReturn &&
      span <= params.ornamentMaxSpanTicks
    ) {
      hypotheses.push({
        class: 'mordent',
