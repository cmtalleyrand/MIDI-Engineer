...
export function getQuantizationWarning(
  ...
  if (
    (!shadowEnabled && options.quantizationValue === 'off' && !options.pruneOverlaps) ||
    selectedTrackIds.size === 0
  )
    return null;
  const quantizationTicks = shadowEnabled
    ? getQuantizationTickValue(options.primaryRhythm.minNoteValue, ppq)
    : getQuantizationTickValue(options.quantizationValue, ppq);
    ...
    track.notes.forEach((n) => {
      ...
      if (quantizationTicks > 0) {
        ...
        if (quantizedDuration < Math.floor(ppq / 32)) {
          microNotesCount++;
          microLocations.add(
            `${n.name} at ${getFormattedTime(ticks, ppq, options.timeSignature.numerator, options.timeSignature.denominator)}`
          );
        }
      }
    });
