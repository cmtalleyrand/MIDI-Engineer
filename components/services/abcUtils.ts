...
export function getAbcPitch(
  midi: number,
  scaleMap: Record<number, ScaleNoteMap>,
  preferFlats: boolean
): string {
  const pc = midi % 12;
...
export function segmentEventsByMeasure(
  events: AbcEvent[],
...
export function determineBestLUnit(
  ...
  for (const ratio of CANDIDATE_L_RATIOS) {
    ...
    if (dominantTicks % lTicks === 0) {
      ...
      if (mult === 1) score += 3000;
      else if (mult === 2) score += 1500;
      else if (mult === 4) score += 800;
      else score -= mult * 20;
    } else {
      score -= 2000;
    }
    let fractionCount = 0;
    notes.forEach((n) => {
      if (n.durationTicks % lTicks !== 0) fractionCount++;
    });
  ...
  return {
    str: bestL.num === 1 ? `1/${bestL.den}` : `${bestL.num}/${bestL.den}`,
    ticks: Math.round(whole * (bestL.num / bestL.den)),
  };
}
