...
export function identifyChord(
  pitches: number[]
): { match: ChordMatch; alternatives: ChordMatch[] } | null {
...
export function detectChordsSustain(
  ...
  validNotes.forEach((n) => {
    points.add(n.ticks);
    points.add(n.ticks + n.durationTicks);
  });
...
export function detectChordsBucketed(
  notes: RawNote[],
  ppq: number,
  tsNum: number,
  tsDenom: number,
  bucketSizeBeats: number
): ChordEvent[] {
...
export function detectChordsHybrid(
  notes: RawNote[],
  ppq: number,
  tsNum: number,
  tsDenom: number,
