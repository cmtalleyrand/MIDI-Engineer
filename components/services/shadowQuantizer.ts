...
function getClosestDuration(
  durationTicks: number,
  quantum: number,
  family: RhythmFamily,
  noteValue: string
): ShadowDurationCandidate {
...
function getConfidenceLabel(
  confidence: ShadowConfidence
): 'Certain' | 'Weak_Primary' | 'Ambiguous' {
 ...
 */
function analyzeShadowCertainty(
  notes: RawNote[],
  ppq: number,
  primary: RhythmRule,
  secondary: RhythmRule
): ShadowNoteAnalysis[] {
  ...
  return notes.map((note) => {
    ...
    if (primaryQuantum > 0) {
      const onset = getClosestGridPoint(
        note.ticks,
        primaryQuantum,
        primary.family,
        primary.minNoteValue
      );
      const duration = getClosestDuration(
        note.durationTicks,
        primaryQuantum,
        primary.family,
        primary.minNoteValue
...
function evaluateHypothesisAtIndex(
  ...
  const conflictTypes: Array<
    'type1_unison_overlap' | 'type2_polyphony_blip' | 'type3_contextual_rhythm'
  > = [];
      ...
      if (overlapAmount > 0) {
        if (note.durationTicks >= existing.durationTicks) {
          ...
          const shortenedExisting = Math.max(
            minDurationTicks,
            existing.durationTicks - overlapAmount
          );
...
function resolveGridConflicts(analyses: ShadowNoteAnalysis[], ppq: number): RawNote[] {
  ...
  analyses.forEach((analysis, idx) => {
    ...
    const evaluated = candidates.map((candidate) =>
      evaluateHypothesisAtIndex(idx, analyses, resolved, candidate, ppq)
    );
 ...
 */
export function applyShadowQuantization(
  notes: RawNote[],
  ppq: number,
  primary: RhythmRule,
  secondary: RhythmRule
