    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
      <div className="bg-gray-800/50 p-5 rounded-lg border border-gray-700">
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-lg font-bold text-gray-light flex items-center gap-2">
            ...
            <span className="text-xs font-normal text-gray-400 px-2 py-0.5 border border-gray-600 rounded-full">
              {detectedGridType}
            </span>
          </h3>
        </div>
        ...
        {stats && (
          <div className="mt-6 pt-4 border-t border-gray-700">
            <div className="flex justify-between items-center mb-2">
              ...
              <span
                className={`text-xs font-mono ${alignmentDelta > 0 ? 'text-green-400' : 'text-gray-500'}`}
              >
                {Math.round(stats.inputGridAlignment * 100)}% &rarr;{' '}
                {Math.round(stats.outputGridAlignment * 100)}% ({deltaSign}
                {Math.round(alignmentDelta * 100)}%)
              </span>
            </div>
            <div className="w-full bg-gray-800 rounded-full h-1.5 flex overflow-hidden">
              <div
                className="bg-gray-500 h-full opacity-50"
                style={{ width: `${stats.inputGridAlignment * 100}%` }}
              ></div>
              {alignmentDelta > 0 && (
                <div
                  className="bg-brand-primary h-full"
                  style={{ width: `${alignmentDelta * 100}%` }}
                ></div>
              )}
            </div>
          </div>
      ...
      <div className="bg-gray-800/30 p-5 rounded-lg border border-gray-700 flex flex-col">
        ...
        {stats ? (
          <>
            <div className="grid grid-cols-2 gap-3 mb-4">
              ...
              />
              <StatItem label="Notes Removed" value={notesRemoved} subtext="Too Short / Overlap" />
              <StatItem
                label="Truncated"
        ...
        <div className="mt-auto grid grid-cols-2 gap-4">
          <div>
            <h4 className="text-[10px] uppercase font-bold text-gray-500 mb-2 border-b border-gray-700 pb-1">
              Input Rhythm
            </h4>
