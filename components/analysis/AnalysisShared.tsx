...
export const ProgressBar: React.FC<{ value: number; label: string; colorClass: string }> = ({
  value,
  label,
  colorClass,
}) => (
      ...
      <div className="flex justify-between items-end mb-1">
        <div className="flex items-center gap-2">
          ...
          <div className="relative">
            <span className="text-[10px] bg-gray-700 text-gray-400 px-1.5 rounded-full cursor-help">
              ?
            </span>
        ...
        <div className="text-right">
          <span
            className={`text-xs font-bold mr-2 px-2 py-0.5 rounded bg-gray-800 ${color.replace('bg-', 'text-')}`}
          >
            {text}
          </span>
...
export const StatItem: React.FC<{
  ...
  onClick?: () => void;
  active?: boolean;
}> = ({ label, value, subtext, highlight, onClick, active }) => (
  <div
    onClick={onClick}
  ...
  >
    ...
    <span className={`text-lg font-bold ${highlight ? 'text-brand-primary' : 'text-gray-200'}`}>
      {value}
    </span>
