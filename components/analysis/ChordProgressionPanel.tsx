...
import {
  ...
  detectChordsSustain,
  detectChordsHybrid,
  detectChordsBucketed,
} from '../services/midiHarmony';
...
export default function ChordProgressionPanel({
  data,
  onRecalculationComplete,
}: ChordProgressionPanelProps) {
  const [chordDetectionMode, setChordDetectionMode] = useState<
    'sustain' | 'attack' | 'hybrid' | 'beat_synced'
  >('attack');
  ...
  useEffect(() => {
    ...
    const toleranceTicks = getTicksFromMusicalValue(
      MUSICAL_TIME_OPTIONS[toleranceIndex].value,
      data.ppq
    );
    const minDurTicks = getTicksFromMusicalValue(
      MUSICAL_TIME_OPTIONS[minNoteDurationIndex].value,
      data.ppq
    );
    let newChords: ChordEvent[] = [];

    if (chordDetectionMode === 'attack') {
      newChords = detectChordsAttack(
        data.notesRaw,
        data.ppq,
      ...
      newChords = detectChordsBucketed(
        data.notesRaw,
        data.ppq,
        data.timeSignature.numerator,
        data.timeSignature.denominator,
  ...
  const chordsToDisplay = showAllChords
    ? visibleChords
    : visibleChords.slice(0, CHORD_DISPLAY_LIMIT);
    ...
    <div className="mt-8 border-t border-gray-700 pt-6">
          ...
          <div className="flex flex-col gap-2 w-full">
              ...
              {chordDetectionMode === 'attack' && (
                <div className="flex items-center gap-3 bg-gray-800 px-3 py-1 rounded-lg border border-gray-700">
                  ...
                  <input
                    type="range"
                    min="0"
                    max={MUSICAL_TIME_OPTIONS.length - 1}
                    step="1"
                    value={toleranceIndex}
                    onChange={(e) => setToleranceIndex(Number(e.target.value))}
                    className="w-20 h-2 bg-gray-600 rounded-lg appearance-none cursor-pointer accent-brand-primary"
                  />
                  <span className="text-xs font-mono text-gray-300 w-12 text-right">
                    {MUSICAL_TIME_OPTIONS[toleranceIndex].label}
                  </span>
                </div>
              )}
              <div className="flex items-center gap-3 bg-gray-800 px-3 py-1 rounded-lg border border-gray-700">
                <span
                  ...
                  title="Ignore notes shorter than this"
                >
                  Min Note Dur:
                </span>
                <input
                  ...
                  max={MUSICAL_TIME_OPTIONS.length - 1}
                  step="1"
                  value={minNoteDurationIndex}
                  onChange={(e) => setMinNoteDurationIndex(Number(e.target.value))}
                  className="w-20 h-2 bg-gray-600 rounded-lg appearance-none cursor-pointer accent-brand-primary"
                />
                <span className="text-xs font-mono text-gray-300 w-12 text-right">
                  {MUSICAL_TIME_OPTIONS[minNoteDurationIndex].label}
                </span>
              </div>
            </div>
            ...
            <div className="flex bg-gray-800 rounded-lg p-1 border border-gray-700 self-end flex-wrap gap-1 justify-end">
              {['sustain', 'attack', 'beat_synced', 'hybrid'].map((mode) => (
                <button
                  key={mode}
                  onClick={() => setChordDetectionMode(mode as any)}
                  className={`px-3 py-1 text-xs font-medium rounded-md transition-colors ${chordDetectionMode === mode ? 'bg-brand-primary text-white shadow' : 'text-gray-400 hover:text-white'}`}
                >
                  {mode === 'beat_synced'
                    ? 'Beat Sync'
      ...
      {chordDetectionMode === 'beat_synced' && (
        <div className="mb-6 p-4 bg-gray-800/50 rounded-lg border border-gray-700 animate-fade-in">
          <h4 className="text-sm font-bold text-gray-300 mb-3 border-b border-gray-700 pb-1">
            Harmonic Rhythm Normalization
          </h4>
            ...
            {[1, 2, 4].map((beat) => (
              <label key={beat} className="flex items-center gap-2 cursor-pointer">
                <input
                  type="radio"
                  name="bucket"
                  checked={bucketSizeBeats === beat}
                  onChange={() => setBucketSizeBeats(beat)}
                  className="text-brand-primary"
                />
                <span className="text-sm text-gray-300">
                  {beat === 4 ? '1 Measure' : `${beat} Beat${beat > 1 ? 's' : ''}`}
                </span>
              </label>
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
            {Array.from({ length: data.voiceCount }).map((_, i) => (
              ...
              >
                ...
                ></div>
                <span className="text-xs font-bold text-gray-300 w-24 truncate">
                  {getVoiceLabel(i, data.voiceCount)}
                </span>
                <select
                  value={voiceConfigs[i] || 'sustain'}
                  onChange={(e) => setVoiceConfigs((prev) => ({ ...prev, [i]: e.target.value }))}
                  className="bg-gray-900 border border-gray-600 text-xs text-gray-200 rounded py-1 px-2 focus:ring-brand-primary focus:border-brand-primary"
                >
          ...
          {arpeggioMode === 'count' && (
            <div className="flex items-center gap-4 bg-gray-900 p-3 rounded border border-gray-700 max-w-md mt-2">
              <input
                ...
                step="1"
                value={arpeggioCount}
                onChange={(e) => setArpeggioCount(Number(e.target.value))}
                className="flex-grow h-2 bg-gray-600 rounded-lg appearance-none cursor-pointer accent-brand-primary"
              />
              <span className="text-xs font-mono text-brand-primary w-20 text-right">
                {arpeggioCount} Notes
              </span>
            </div>
          )}
        ...
        <table className="w-full text-sm text-left text-gray-400">
          <thead className="text-xs text-gray-300 uppercase bg-gray-700 sticky top-0">
            <tr>
              ...
              <th className="px-6 py-3">Chord</th>
            ...
            {chordsToDisplay.length > 0 ? (
              chordsToDisplay.map((chord, i) => (
                <React.Fragment key={i}>
                  <tr
                    className={`border-b border-gray-700 hover:bg-gray-700/50 cursor-pointer ${expandedChordIndex === i ? 'bg-gray-700/30' : ''}`}
                    onClick={() => setExpandedChordIndex(expandedChordIndex === i ? null : i)}
                  >
                    <td className="px-6 py-4 text-center">
                      {chord.alternatives?.length > 0 &&
                        (expandedChordIndex === i ? <ChevronUpIcon /> : <ChevronDownIcon />)}
                    </td>
                    ...
                    <td className="px-6 py-4 text-xs text-gray-500 font-mono">
                      ...
                      {chord.missingNotes.length > 0 && (
                        <span className="text-amber-500 ml-2">
                          (Missing: {chord.missingNotes.join(', ')})
                        </span>
                      )}
                    </td>
                  </tr>
                  {expandedChordIndex === i && chord.alternatives?.length > 0 && (
                    <tr className="bg-gray-800/80">
                      <td colSpan={4} className="px-6 py-3">
                        <div className="space-y-2 pl-4 border-l-2 border-brand-primary/50">
                          {chord.alternatives.map((alt, altIdx) => (
                            <div key={altIdx} className="flex items-center gap-4 text-xs">
                              <span className="font-bold text-gray-300 w-32">{alt.name}</span>
                              <span className="text-gray-500">Score: {alt.score}</span>
                            </div>
                          ))}
                        </div>
                      </td>
              ...
              <tr>
                <td colSpan={4} className="px-6 py-8 text-center text-gray-500 italic">
                  No chords detected with current settings.
                </td>
              </tr>
            )}
          </tbody>
        ...
        {visibleChords.length > CHORD_DISPLAY_LIMIT && (
          <div className="p-2 bg-gray-700/50 text-center border-t border-gray-700">
            <button
              ...
              className="text-xs font-bold text-brand-primary hover:text-white transition-colors uppercase tracking-wider px-4 py-2"
            >
