...
export default function QuantizationSettings({ quantizationWarning }: QuantizationSettingsProps) {
  ...
  const {
    setPrimaryRhythm,
    setSecondaryRhythm,
    setQuantizeDurationMin,
    setShiftToMeasure,
    setPruneOverlaps,
    setPruneThresholdIndex,
  } = setters;
  ...
  return (
    <div className="border-t border-gray-medium pt-4">
      ...
      <div className="space-y-6">
        ...
        <div className="bg-gray-800 p-4 rounded-lg border border-gray-700">
          <div className="flex items-center justify-between mb-3">
            ...
            {primaryRhythm.enabled && (
              <span className="text-xs text-brand-primary font-mono uppercase">Active</span>
            )}
          </div>
        ...
        {/* Secondary Rhythm */}
        <div
          className={`bg-gray-800 p-4 rounded-lg border border-gray-700 transition-opacity ${!primaryRhythm.enabled ? 'opacity-50 pointer-events-none' : ''}`}
        >
          <div className="flex items-center justify-between mb-3">
            <label className="flex items-center cursor-pointer">
              <input
          ...
          <p className="text-xs text-gray-500 mt-2">
            Allows notes to snap to this grid if they don't fit the Primary grid.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-400 mb-1">
              Minimum Note Value (Output)
            </label>
            <select
              id="quantizeDurationMin"
              value={quantizeDurationMin}
              onChange={(e) => setQuantizeDurationMin(e.target.value)}
              className="block w-full bg-gray-darker border border-gray-medium rounded-md shadow-sm py-2 px-3 sm:text-sm text-gray-light"
            >
              <option value="off">Same as Grid (Auto)</option>
              {MUSICAL_TIME_OPTIONS.filter((o) => o.value > 0).map((opt) => (
                <option key={opt.value} value={opt.value}>
                  {opt.label}
                </option>
              ))}
            ...
            <p className="text-xs text-gray-500 mt-1">
              Staccato notes shorter than this will be extended.
            </p>
          </div>
          ...
          <div className="flex items-end">
            <label className="flex items-center p-3 bg-gray-darker rounded-lg border border-gray-medium hover:border-brand-secondary/50 transition-colors cursor-pointer h-full w-full">
              <input
                type="checkbox"
                checked={shiftToMeasure}
                onChange={(e) => setShiftToMeasure(e.target.checked)}
          ...
          <div className="md:col-span-2 p-4 bg-gray-darker rounded-lg border border-gray-medium">
            <label className="flex items-center cursor-pointer mb-3">
              <input
                type="checkbox"
                checked={pruneOverlaps}
                onChange={(e) => setPruneOverlaps(e.target.checked)}
                className="h-5 w-5 rounded bg-gray-dark border-gray-medium text-brand-primary focus:ring-brand-primary focus:ring-2"
              />
              <div className="ml-3">
                <span className="font-semibold text-gray-light">Prune Pitch Overlaps</span>
                <p className="text-xs text-gray-400">
                  Ensures monophony per pitch (clean sheet music).
                </p>
              </div>
            </label>
            {pruneOverlaps && (
              <div className="flex items-center gap-4 mt-4 border-t border-gray-700 pt-3">
                ...
                <input
                  ...
                  step="1"
                  value={pruneThresholdIndex}
                  onChange={(e) => setPruneThresholdIndex(Number(e.target.value))}
                ...
                <span className="text-xs font-mono text-brand-primary w-12 text-right">
                  {MUSICAL_TIME_OPTIONS[pruneThresholdIndex].label}
                </span>
              </div>
            )}
          </div>
          ...
          <div className="mt-2 p-3 bg-yellow-900/50 border border-yellow-700 text-yellow-200 rounded-lg text-sm flex flex-col gap-2">
            ...
            {quantizationWarning.details.length > 0 && (
              <button
                ...
                className="text-xs text-yellow-500 hover:text-white underline font-bold"
              >
            ...
            {showWarningDetails && (
              <div className="max-h-32 overflow-y-auto bg-black/30 p-2 rounded border border-yellow-700/50 font-mono text-[10px]">
                {quantizationWarning.details.map((d, idx) => (
                  <div key={idx} className="text-yellow-400 opacity-80">
                    {d}
                  </div>
                ))}
              </div>
