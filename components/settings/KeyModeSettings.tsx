...
export default function KeyModeSettings() {
  ...
  const {
    setModalRoot,
    setModalModeName,
    setIsModalConversionEnabled,
    setModalMappings,
    setKeySignatureSpelling,
    setAbcKeyExport,
  } = setters;
      ...
      <div className="bg-gray-darker p-4 rounded-lg border border-gray-medium">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
          ...
          <div>
            ...
            <select
              value={modalModeName}
              onChange={(e) => setModalModeName(e.target.value)}
              className="block w-full bg-gray-800 border border-gray-600 rounded-md py-2 px-3 text-gray-light"
            >
          ...
          <div className="flex items-center justify-between">
            <div>
              <span className="text-sm font-medium text-gray-300">
                Override ABC export key (K:)
              </span>
              <p className="text-xs text-gray-500 mt-1">
                Choose tonic, mode and additional accidentals using ABC key syntax.
              </p>
            </div>
            <label className="flex items-center cursor-pointer">
              ...
              />
              <div
                className={`block w-10 h-6 rounded-full transition-colors ${abcKeyExport.enabled ? 'bg-brand-primary' : 'bg-gray-700'}`}
              ></div>
            </label>
          </div>
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setAbcKeyExport({
                        ...abcKeyExport,
                        tonicLetter: e.target.value as (typeof ABC_TONIC_LETTERS)[number],
                      })
                    }
                    className="block w-full bg-gray-800 border border-gray-600 rounded-md py-2 px-3 text-gray-light"
                  >
                    {ABC_TONIC_LETTERS.map((letter) => (
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setAbcKeyExport({
                        ...abcKeyExport,
                        tonicAccidental: e.target.value as (typeof ABC_ACCIDENTALS)[number],
                      })
                    }
                    className="block w-full bg-gray-800 border border-gray-600 rounded-md py-2 px-3 text-gray-light"
                  >
                    {ABC_ACCIDENTALS.map((acc) => (
                    ...
                    onClick={() =>
                      setAbcKeyExport({
                        ...
                        additionalAccidentals: [
                          ...abcKeyExport.additionalAccidentals,
                          { accidental: '^', letter: 'f' },
                        ],
                      })
                    }
                    ...
                    >
                      <select
                        ...
                        onChange={(e) => {
                          ...
                          next[index] = {
                            ...next[index],
                            letter: e.target.value as (typeof ABC_NOTE_LETTERS)[number],
                          };
                      ...
                      <select
                        ...
                        onChange={(e) => {
                          ...
                          next[index] = {
                            ...next[index],
                            accidental: e.target.value as (typeof ABC_ACCIDENTALS)[number],
                          };
                          setAbcKeyExport({ ...abcKeyExport, additionalAccidentals: next });
                        }}
                        className="bg-gray-800 border border-gray-600 rounded-md py-2 px-3 text-gray-light"
                      ...
                      <button
                        ...
                        onClick={() => {
                          setAbcKeyExport({
                            ...abcKeyExport,
                            additionalAccidentals: abcKeyExport.additionalAccidentals.filter(
                              (_, i) => i !== index
                            ),
                          });
                        }}
        ...
        <div className="flex items-center justify-between mt-6 pt-4 border-t border-gray-700">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">
              Allows you to remap notes from the source key to a target scale (e.g. Major to Minor
              conversion).
            </p>
          </div>
          <label className="flex items-center cursor-pointer">
            <div className="relative">
              <input
                type="checkbox"
                className="sr-only"
                checked={isModalConversionEnabled}
                onChange={(e) => setIsModalConversionEnabled(e.target.checked)}
              />
              <div
                className={`block w-10 h-6 rounded-full transition-colors ${isModalConversionEnabled ? 'bg-brand-primary' : 'bg-gray-700'}`}
              ></div>
              <div
                className={`dot absolute left-1 top-1 bg-white w-4 h-4 rounded-full transition-transform ${isModalConversionEnabled ? 'transform translate-x-4' : ''}`}
              ></div>
            </div>
          </label>
        </div>
            ...
            <table className="w-full text-sm text-left text-gray-400">
              <thead>
                <tr>
                  ...
                  {midiPitchHistogram && (
                    <th className="px-2 py-2 text-xs text-gray-500 text-right">In</th>
                  )}
                  <th></th>
                  <th className="px-4 py-2">Target</th>
                  {midiPitchHistogram && (
                    <th className="px-2 py-2 text-xs text-gray-500 text-right">Out</th>
                  )}
                </tr>
              </thead>
              <tbody>
                {Array.from({ length: 12 }).map((_, i) => (
                  <tr
                    key={i}
                    className={`border-b border-gray-700 ${modeIntervals.includes(i) ? 'bg-gray-800/50' : ''}`}
                  >
                    <td className="px-4 py-2 font-medium">
                      {getNoteName(modalRoot, i)}{' '}
                      <span className="text-xs text-gray-500">
                        ({getDegreeLabel(i, modeIntervals)})
                      </span>
                    </td>
                    {midiPitchHistogram && (
                      <td className="px-2 py-2 text-right">
                        <span
                          className={`text-xs font-mono ${(midiPitchHistogram[(modalRoot + i) % 12] || 0) > 0 ? 'text-blue-400' : 'text-gray-600'}`}
                        >
                    ...
                    <td className="px-4 py-2">
                      <select
                        ...
                        onChange={(e) =>
                          setModalMappings({ ...modalMappings, [i]: Number(e.target.value) })
                        }
                      ...
                      >
                        {Array.from({ length: 12 }).map((_, tIdx) => (
                          <option key={tIdx} value={tIdx}>
                            {getNoteName(modalRoot, tIdx)} - {getDegreeLabel(tIdx, modeIntervals)}
                          </option>
                        ))}
                      </select>
                    </td>
                    {midiPitchHistogram && outputHistogram && (
                      <td className="px-2 py-2 text-right">
                        <span
                          className={`text-xs font-mono ${(outputHistogram[modalMappings[i] ?? i] || 0) > 0 ? 'text-green-400' : 'text-gray-600'}`}
                        >
            ...
            {midiPitchHistogram && (
              <div className="mt-3 px-2 text-xs text-gray-500">
                {(() => {
                  ...
                  const total = (Object.values(midiPitchHistogram) as number[]).reduce(
                    (a, b) => a + b,
                    0
                  );
                  return moved > 0 ? (
                    <span className="text-yellow-500">
                      {moved} of {total} notes will be remapped
                    </span>
                  ) : (
