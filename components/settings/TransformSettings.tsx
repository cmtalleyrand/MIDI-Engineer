...
export default function TransformSettings({ inversionStats }: TransformSettingsProps) {
  const { settings, setters } = useSettings();
  const {
    transpositionSemitones,
    transpositionOctaves,
    noteTimeScale,
    inversionMode,
    melodicInversion,
    detectOrnaments,
    removeShortNotesThresholdIndex,
  } = settings;
  const {
    setTranspositionSemitones,
    setTranspositionOctaves,
    setNoteTimeScale,
    setInversionMode,
    setMelodicInversion,
    setDetectOrnaments,
  ...
  return (
    <div className="border-t border-gray-medium pt-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
          ...
          <div className="flex items-center gap-3">
            <div className="flex-1">
              <input
                ...
                onChange={(e) => setTranspositionSemitones(e.target.value)}
                min="-11"
                max="11"
                className="w-full bg-gray-darker border border-gray-medium rounded-md py-2 px-3 text-center sm:text-sm text-gray-light"
              />
            ...
            <div className="flex-1">
              <input
                type="number"
                value={transpositionOctaves}
                onChange={(e) => setTranspositionOctaves(e.target.value)}
                min="-5"
                max="5"
                className="w-full bg-gray-darker border border-gray-medium rounded-md py-2 px-3 text-center sm:text-sm text-gray-light"
              />
        ...
        <div>
          <label htmlFor="note-time-scale" className="block text-sm font-medium text-gray-400 mb-1">
            Augmentation / Diminution
          </label>
          <select
            id="note-time-scale"
            value={noteTimeScale}
            onChange={(e) => setNoteTimeScale(e.target.value)}
            className="block w-full bg-gray-darker border border-gray-medium rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-brand-primary focus:border-brand-primary sm:text-sm text-gray-light"
          >
          ...
          <p className="text-xs text-gray-500 mt-1">
            Scales note durations. &lt;1 is faster (Diminution), &gt;1 is slower (Augmentation).
          </p>
        </div>
        ...
        <div className="md:col-span-2">
          ...
          <div className="flex items-center gap-4 bg-gray-darker p-3 rounded-lg border border-gray-medium hover:border-brand-secondary/50 transition-colors">
            ...
            />
            <span className="text-xs font-mono text-brand-primary w-12 text-right">
              {MUSICAL_TIME_OPTIONS[removeShortNotesThresholdIndex].label}
            </span>
          </div>
          <p className="text-xs text-gray-500 mt-1">
            Filters out ghost notes or glitches below this duration.
          </p>
        </div>

        {/* Retrograde Section */}
        <div className="bg-gray-800 p-4 rounded-lg border border-gray-700">
          <label htmlFor="inversion-mode" className="block text-sm font-bold text-gray-300 mb-1">
            Retrograde (Time Reversal)
          </label>
          <select
            id="inversion-mode"
            value={inversionMode}
            onChange={(e) => setInversionMode(e.target.value as InversionMode)}
            className="block w-full bg-gray-900 border border-gray-600 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-brand-primary focus:border-brand-primary sm:text-sm text-gray-light"
          >
        ...
        <div className="bg-gray-800 p-4 rounded-lg border border-gray-700 flex flex-col">
            ...
            <div className="space-y-4 animate-fade-in flex-grow">
              ...
              {inversionStats ? (
                <div className="bg-gray-900/50 p-3 rounded border border-gray-700 text-xs">
                  <h4 className="font-bold text-gray-400 mb-2 uppercase tracking-wider text-[10px]">
                  ...
                  <div className="space-y-1">
                    ...
                    {inversionStats.hasPolyphony && (
                      <div className="mt-2 text-yellow-500 bg-yellow-900/20 p-1.5 rounded border border-yellow-800/50">
                        <strong>Warning:</strong> Selected range contains chords (polyphony).
                        Inversion may produce voice crossings.
                      </div>
                    )}
                    {inversionStats.totalNotes === 0 && (
                      <div className="mt-2 text-red-400 italic">
                        No notes found in measures {melodicInversion.startMeasure}-
                        {melodicInversion.endMeasure}.
                      </div>
                    )}
                  </div>
                </div>
          ...
          <p className="text-xs text-gray-500 mt-2">
            Inverts intervals relative to the <strong>first note</strong> in the selection.
          </p>
        </div>
          ...
          <label className="flex items-center p-3 bg-gray-darker rounded-lg border border-gray-medium hover:border-brand-secondary/50 transition-colors cursor-pointer w-full">
            ...
            <div className="ml-3">
              ...
              <p className="text-xs text-gray-400">
                Detects Trills, Turns, and Grace Notes and keeps them attached to their principal
                note during processing.
              </p>
            </div>
          </label>
        </div>
